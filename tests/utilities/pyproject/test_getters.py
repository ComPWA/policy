...
def describe_get_project_urls():
    def returns_urls_and_source():
        src = """
            [project]
            name = "my-package"
            [project.urls]
...
def describe_get_supported_python_versions():
    def reads_from_classifiers():
        src = """
            [project]
            name = "my-package"
            classifiers = [
...
def describe_get_sub_table():
    def returns_nested_tables_and_values():
        ...
        with pytest.raises(
            KeyError, match=r"TOML data does not contain 'non-existent'"
        ):
