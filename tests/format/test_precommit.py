...
def test_sort_hooks_orders_all_categories():
    with (
        pytest.raises(PrecommitError, match=r"Sorted all pre-commit hooks"),
        _load("""
            repos:
...
def test_ci_updates_skip_without_ci_section_are_noops():
    with _load("""
        repos:
          - repo: meta
            hooks:
...
def test_update_precommit_ci_autofix_commit_msg():
    with (
...
def test_update_precommit_ci_skip_collects_local_and_non_functional_hooks():
    with (
...
def test_update_precommit_ci_skip_removes_redundant_section():
    with (
...
def test_get_local_and_non_functional_hooks():
...
def test_update_conda_environment_sets_legacy_flag(
...
def test_update_conda_environment_removes_legacy_flag(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "environment.yml").write_text("variables:\n  PRETTIER_LEGACY_CLI: 1\n")
...
def test_main_sorts_and_updates(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.chdir(tmp_path)
