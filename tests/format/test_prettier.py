...
def test_update_prettier_hook_renames_mirror():
    with (
...
def test_update_prettier_hook_without_mirror():
...
def test_remove_configuration_removes_files(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".prettierrc.json").write_text("{}")
    (tmp_path / ".prettierrc").write_text("{}")
    with pytest.raises(PrecommitError, match=r"Removed redundant configuration files"):
        _remove_configuration()
    assert not (tmp_path / ".prettierrc.json").exists()
    assert not (tmp_path / ".prettierrc").exists()


def test_remove_configuration_without_files(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.chdir(tmp_path)
...
def test_update_prettier_ignore_removes_forbidden(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.chdir(tmp_path)
...
def test_update_prettier_ignore_inserts_obligatory(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.chdir(tmp_path)
...
def test_update_prettier_ignore_removes_empty_file(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.chdir(tmp_path)
...
def test_main_with_prettier_repo(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "README.md").write_text("# Title\n")
    with (
...
def test_main_without_prettier_repo(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "README.md").write_text("# Title\n")
    (tmp_path / ".prettierrc.json").write_text("{}")
    with (
