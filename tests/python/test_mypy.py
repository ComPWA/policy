...
def describe_merge_mypy_into_pyproject():
...
def describe_remove_mypy():
    def removes_hook_and_config():
    ...
    def is_noop_without_configuration_table():
        with (
...
def describe_update_vscode_settings():
    def configures_extension_when_active(
    ...
    def marks_extension_unwanted_when_inactive(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        monkeypatch.chdir(tmp_path)
        with pytest.raises(PrecommitError):
